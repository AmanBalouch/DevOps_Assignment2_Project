...
const BookingCalendar = ({ property, onBookingSuccess }) => {
  ...
  const fetchExistingBookings = async () => {
    try {
      ...
      const response = await fetch(`${window.location.protocol}//${window.location.hostname}:8001/api/bookings/property/${property.id}/bookings`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await fetch(`${window.location.protocol}//${window.location.hostname}:8001/api/bookings/`, {
        method: 'POST',
        headers: {
